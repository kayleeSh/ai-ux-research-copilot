...
export function NextStepBanner({ icon, title, description, to, buttonText }: NextStepBannerProps) {
  return (
    <div
      ...
      style={{ marginTop: 32, padding: '18px 24px', borderLeft: '3px solid #4f46e5', background: '#f8faff' }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
          <span style={{ fontSize: '1.4rem', flexShrink: 0 }}>{icon}</span>
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#4f46e5', marginBottom: 4 }}>
              Next Step
            </div>
